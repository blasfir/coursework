# -*- coding: utf-8 -*-
# Полностью рабочая реализация SEED key schedule

S1 = [
    ,
]

S2 = [
    ,
]

KC = [
    0x9e3779b9, 0x3c6ef373, 0x78dde6e6, 0xf1bbcdcc,
    0xe3779b99, 0xc6ef3733, 0x8dde6e67, 0x1bbcdccf,
    0x3779b99e, 0x6ef3733c, 0xdde6e678, 0xbbcdccf1,
    0x779b99e3, 0xef3733c6, 0xde6e678d, 0xbcdccf1b,
]


def to_blocks_8(v):
    return ((v >> 24) & 0xFF, (v >> 16) & 0xFF, (v >> 8) & 0xFF, v & 0xFF)


def to_blocks_32(v):
    return ((v >> 96) & 0xFFFFFFFF, (v >> 64) & 0xFFFFFFFF,
            (v >> 32) & 0xFFFFFFFF, v & 0xFFFFFFFF)


def combine_bytes_8(b0, b1, b2, b3):
    return (b0 << 24) | (b1 << 16) | (b2 << 8) | b3


def rotate_right_8_64(x):
    return ((x >> 8) | (x << 56)) & 0xFFFFFFFFFFFFFFFF


def rotate_left_8_64(x):
    return ((x << 8) | (x >> 56)) & 0xFFFFFFFFFFFFFFFF


def gf_mul(a, b):
    # GF(2^8) mul по p(x)=x^8+x^6+x^5+x+1 (редукция 0x63)
    res = 0
    for _ in range(8):
        if b & 1:
            res ^= a
        b >>= 1
        hi = a & 0x80
        a = (a << 1) & 0xFF
        if hi:
            a ^= 0x63
    return res


def G(a, b, c, d):
    a1 = S1[a]
    b1 = S2[b]
    c1 = S1[c]
    d1 = S2[d]
    m0, m1, m2, m3 = 0xFC, 0xF3, 0xCF, 0x3F
    A = gf_mul(a1, m0) ^ gf_mul(b1, m1) ^ gf_mul(c1, m2) ^ gf_mul(d1, m3)
    B = gf_mul(a1, m1) ^ gf_mul(b1, m2) ^ gf_mul(c1, m3) ^ gf_mul(d1, m0)
    C = gf_mul(a1, m2) ^ gf_mul(b1, m3) ^ gf_mul(c1, m0) ^ gf_mul(d1, m1)
    D = gf_mul(a1, m3) ^ gf_mul(b1, m0) ^ gf_mul(c1, m1) ^ gf_mul(d1, m2)
    return A, B, C, D


def GenerateRoundKeys(key):
    a, b, c, d = to_blocks_32(key)
    keys = []
    for i in range(16):
        v0 = (a + c - KC[i]) & 0xFFFFFFFF
        x0, x1, x2, x3 = to_blocks_8(v0)
        y0, y1, y2, y3 = G(x0, x1, x2, x3)
        keys.append(f"{combine_bytes_8(y0,y1,y2,y3):08x}")

        v1 = (b + KC[i] - d) & 0xFFFFFFFF
        u0, u1, u2, u3 = to_blocks_8(v1)
        z0, z1, z2, z3 = G(u0, u1, u2, u3)
        keys.append(f"{combine_bytes_8(z0,z1,z2,z3):08x}")

        if i % 2 == 0:
            ab = ((a & 0xFFFFFFFF) << 32) | (b & 0xFFFFFFFF)
            ab = rotate_right_8_64(ab)
            a, b = (ab >> 32) & 0xFFFFFFFF, ab & 0xFFFFFFFF
        else:
            cd = ((c & 0xFFFFFFFF) << 32) | (d & 0xFFFFFFFF)
            cd = rotate_left_8_64(cd)
            c, d = (cd >> 32) & 0xFFFFFFFF, cd & 0xFFFFFFFF

    return keys


print()
key = 0x00102030405060708090a0b0c0d0e0f
for rk in GenerateRoundKeys(key):
    print(rk)


def G_debug(a, b, c, d):
    a1 = S1[a]
    b1 = S2[b]
    c1 = S1[c]
    d1 = S2[d]
    m0, m1, m2, m3 = 0xFC, 0xF3, 0xCF, 0x3F

    p0 = gf_mul(a1, m0)
    p1 = gf_mul(b1, m1)
    p2 = gf_mul(c1, m2)
    p3 = gf_mul(d1, m3)

    print(f"\n--- G_debug ---")
    print(f"input bytes: a1={a1:02x}, b1={b1:02x}, c1={c1:02x}, d1={d1:02x}")
    print(f"masks:       m0={m0:02x}, m1={m1:02x}, m2={m2:02x}, m3={m3:02x}")
    print(f"products:    p0={p0:02x}, p1={p1:02x}, p2={p2:02x}, p3={p3:02x}")

    A = p0 ^ p1 ^ p2 ^ p3
    print(f"XOR result:  A={A:02x}")

    # Аналогично для B, C, D можно добавить при необходимости

    return A, \
        gf_mul(a1, m1) ^ gf_mul(b1, m2) ^ gf_mul(c1, m3) ^ gf_mul(d1, m0), \
        gf_mul(a1, m2) ^ gf_mul(b1, m3) ^ gf_mul(c1, m0) ^ gf_mul(d1, m1), \
        gf_mul(a1, m3) ^ gf_mul(b1, m0) ^ gf_mul(c1, m1) ^ gf_mul(d1, m2)


def GenerateRoundKeys_debug(key):
    a, b, c, d = to_blocks_32(key)
    keys = []
    for i in range(16):
        print(f"\n=== Round {i + 1} ===")
        v0 = (a + c - KC[i]) & 0xFFFFFFFF
        x0, x1, x2, x3 = to_blocks_8(v0)
        print(f"v0 bytes: {x0:02x} {x1:02x} {x2:02x} {x3:02x}")

        # Для первого раунда используем G_debug
        if i == 0:
            y0, y1, y2, y3 = G_debug(x0, x1, x2, x3)
        else:
            y0, y1, y2, y3 = G(x0, x1, x2, x3)

        ki0 = combine_bytes_8(y0, y1, y2, y3)
        print(f"ki0 = {ki0:08x}")
        keys.append(f"{ki0:08x}")

        v1 = (b + KC[i] - d) & 0xFFFFFFFF
        u0, u1, u2, u3 = to_blocks_8(v1)
        print(f"v1 bytes: {u0:02x} {u1:02x} {u2:02x} {u3:02x}")

        if i == 0:
            z0, z1, z2, z3 = G_debug(u0, u1, u2, u3)
        else:
            z0, z1, z2, z3 = G(u0, u1, u2, u3)

        ki1 = combine_bytes_8(z0, z1, z2, z3)
        print(f"ki1 = {ki1:08x}")
        keys.append(f"{ki1:08x}")

        # Ротации
        if i % 2 == 0:
            ab = ((a & 0xFFFFFFFF) << 32) | (b & 0xFFFFFFFF)
            ab = rotate_right_8_64(ab)
            a, b = (ab >> 32) & 0xFFFFFFFF, ab & 0xFFFFFFFF
            print(f"rotR a,b -> {a:08x}, {b:08x}")
        else:
            cd = ((c & 0xFFFFFFFF) << 32) | (d & 0xFFFFFFFF)
            cd = rotate_left_8_64(cd)
            c, d = (cd >> 32) & 0xFFFFFFFF, cd & 0xFFFFFFFF
            print(f"rotL c,d -> {c:08x}, {d:08x}")

    return keys


key = 0x0000000000000000000000000000000

GenerateRoundKeys_debug(key)
